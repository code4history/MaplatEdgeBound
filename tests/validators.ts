...
export function validateDelaunator(points: PTS, del: DelaunatorLike) {
    ...
    for(let edg = 0; edg < del.triangles.length; edg++){
        ...
        if(del.halfedges[adj] !== edg){
            throw new Error(`invalid halfedge connection: ${edg} -> ${adj} != ${del.halfedges[adj]} -> ${edg}`);
        }
        ...
        if(e1 !== a2 && e2 !== a1){
            throw new Error(`halfedges ${edg}/${adj} do not share end-points (${e1}, ${e2}) / (${a2}, ${a1})`);
        }
    }
...
export function validateVertMap(points: PTS, con: Constrainautor) {
    const del = con.del,
...
export function validateFlips(con: Constrainautor, clear = true) {
    const del = con.del,
        numEdges = del.triangles.length;
    ...
    for(let edg = 0; edg < numEdges; edg++){
        ...
        if(con.isConstrained(edg) !== con.isConstrained(adj) || 
           con.flips.has(edg) !== con.flips.has(adj)){
            throw new Error(`flip status inconsistent for ${edg}/${adj}: ${con.flips.has(edg)}/${con.flips.has(adj)}`);
        }
    }
}

export function validateDelaunay(con: Constrainautor) {
    const del = con.del,
        pts = del.coords,
...
export function validateConstraint(points: PTS, con: Constrainautor, ret: number | undefined, p1: number, p2: number) {
    const del = con.del,
        numEdges = del.triangles.length,
        [x1, y1] = points[p1],
        [x2, y2] = points[p2],
    ...
    if(ret !== undefined && (re1 !== p1 || re2 !== p2)){
        throw new Error(`invalid edge returned from constrainOne: ${ret}: ${re1} -> ${re2} !== ${p1} -> ${p2}`);
    }
    ...
    for(let edg = 0; edg < numEdges; edg++){
        ...
        if(robustIntersect([x1, y1], [x2, y2], [x3, y3], [x4, y4])){
            throw new Error(`edge ${edg} (${e1} -> ${e2}) intersects constrained edge ${p1} -> ${p2}`);
        }
    }
...
export function validateAllConstraints(points: PTS, edges: [number, number][], con: Constrainautor) {
    ...
    for(let edg = 0; edg < numEdges; edg++){
        if(conEdges.has(edg)){
            if(!con.isConstrained(edg)){
                throw new Error(`constrained edge ${edg} (${triangles[edg]} -> ${triangles[nextEdge(edg)]}) not marked as constrained`);
            }
        }else if(con.isConstrained(edg)){
            throw new Error(`non-constrained edge ${edg} (${triangles[edg]} -> ${triangles[nextEdge(edg)]}) marked as constrained`);
        }
    }
}
